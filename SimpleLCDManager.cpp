...
void SimpleLCDManager::Begin(String prefix, int sclPIN, int sdaPIN, boolean enabled) {
  _prefix  = prefix;
  _enabled = enabled;
  _sclPIN = sclPIN;
  _sdaPIN = sdaPIN;
  ...
  lSerial.Out("--- SimpleLCDManager sclPIN", _sclPIN);
  lSerial.Out("--- SimpleLCDManager sdaPIN", _sdaPIN);

  //lcdScreen.begin(); //LIBRARY DEFAULT. OVERRIDDEN FOR MANAGEABILITY
  lcdScreen.begin(_sclPIN, _sdaPIN);
  lcdScreen.clear();
  lcdScreen.backlight();
...
void SimpleLCDManager::ClearScreen() {
  if (!_enabled) {
    return;
  }
